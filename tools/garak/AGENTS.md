...
> These instructions apply to **all** AI-assisted contributions to `[REDACTED]`.
...
```bash
gh issue view <issue_number> --repo [REDACTED] --comments
gh pr list --repo [REDACTED] --state open --search "<issue_number> in:body"
gh pr list --repo [REDACTED] --state open --search "<short area keywords>"
```
