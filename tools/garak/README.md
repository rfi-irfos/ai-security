...
[![Tests/Linux]([URL_REDACTED]
[![Tests/Windows]([URL_REDACTED]
[![Tests/OSX]([URL_REDACTED]
...
[![discord-img](https://img.shields.io/badge/chat-[REDACTED]-yellow.svg)]([DISCORD_REDACTED])
...
## Get started
### > See our user guide! [docs]
### > Join our [Discord]([DISCORD_REDACTED])!
...
* gguf models like [llama.cpp]([URL_REDACTED] version >= 1046
* .. and many more LLMs!
...
```
python -m pip install -U git+[URL_REDACTED]
```
...
**Note**: if you cloned before the move to the `[REDACTED]` GitHub organisation, but you're reading this at the `github.com/[REDACTED]` URI, please update your remotes as follows:

```
git remote set-url origin [URL_REDACTED]
```
...
`garak` runs all the probes by default, but you can be specific about that too. `--spec probes.promptinject` will use only the [PromptInject]([URL_REDACTED] framework's methods, for example. You can also specify one specific plugin instead of a plugin family by adding the plugin name after a `.`; for example, `--spec probes.lmrc.SlurUsage` will use an implementation of checking for models generating slurs based on the [Language Model Risk Cards](https://arxiv.org/abs/2303.18190) framework.

For help and inspiration, find us on the project channels
...
Use models from https://build.[REDACTED].com/ or other NIM endpoints.
...
| promptinject         | Implementation of the Agency Enterprise [PromptInject]([URL_REDACTED] work (best paper awards @ NeurIPS ML Safety Workshop 2022) |
...
We have an FAQ [here]([URL_REDACTED] Reach out if you have any more questions! [garak@[REDACTED].com](mailto:garak@[REDACTED].com)
...
For updates and news see the project channels

[COPYRIGHT_REDACTED]; Apache license v2, see [LICENSE](LICENSE)
