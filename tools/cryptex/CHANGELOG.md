...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.8.1` (`linux/amd64`, plus `linux/arm64` when the emulated build completes within its cap). `:latest`, `:v2.8`, `:2.8` track this tag.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.7.1` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.7`, `:2.7` track this tag.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.7.0` (multi-arch `linux/amd64` plus `linux/arm64`). `:latest`, `:v2.7`, `:2.7`, `:v2`, `:2` all point at the same SHA.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.6.1` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.6`, `:2.6`, `:v2`, `:2` all point at the same SHA.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.6.0` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.6`, `:2.6`, `:v2`, `:2` all point at the same SHA.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.5.0` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.5`, `:2.5`, `:v2`, `:2` all point at the same SHA. **The new pipeline guarantees `:latest` tracks the v2.5.0 SHA going forward — the bug that caused users to pull stale code is fixed.**
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.4.2` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.4`, `:2.4`, `:v2`, `:2` all point at the same SHA.
...
- `package.json` — `homepage` changed from `cryptex.m4xx.cfd` (sibling product) to `[URL_REDACTED] (this repo). Dead test scripts dropped (`test:universal`, `test:steg`, `test:lexeme`, `test:lexeme-ui`, `test:all`); `npm test` now aliases `cd app && npx vitest run`.
...
- README footer — a one-line P.S. crediting [elder-plinius/P4RS3LT0NGV3]([URL_REDACTED] as the spiritual ancestor of the genre. Cryptex OSS was built from scratch; this is a hat-tip, not a derivation.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.4.0` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.4`, `:2.4`, `:v2`, `:2` all point at the same SHA.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.3.0` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.3`, `:2.3`, `:v2`, `:2` all point at the same SHA.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.2.0` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.2`, `:2.2`, `:v2`, `:2` all point at the same SHA.
...
- `ghcr.io/[REDACTED]/cryptex-oss:v2.1.1` (multi-arch `linux/amd64` + `linux/arm64`). `:latest`, `:v2.1`, `:v2`, `:main` all point at the same SHA.
...
- `docker run -d -p 8080:80 ghcr.io/[REDACTED]/cryptex-oss:latest` one-line install.
...
- `docker-compose.yml` carries a commented `image: ghcr.io/[REDACTED]/cryptex-oss:latest` alternate so Dokploy users can swap from build-from-source to pull-prebuilt by uncommenting one line.

### Changed
- Dockerfile OCI labels point at `github.com/[REDACTED]/cryptex-oss` (was the closed-source `cryptex` URL). Title and description reflect OSS framing. Adds `org.opencontainers.image.documentation` and `org.opencontainers.image.vendor`.
...
- **Local provider support** without BYOK keys: Ollama, LM Studio, vLLM, llama.cpp, Llamafile, [REDACTED] NIM.
